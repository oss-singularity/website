...
The next infrastructure milestone is [release automation](docs/release-automation.md): a reviewed change should be publishable through the shared project workflow, with exact artifacts, scoped deployment access and recoverable failures. The [static artifact verifier](docs/release-artifacts.md), [GitHub rehearsal](docs/release-rehearsal.md) and independent [candidate consumer](docs/release-candidates.md) cover bytes, transport and completed-run handoff. Required-check policy and production automation remain separate work.
