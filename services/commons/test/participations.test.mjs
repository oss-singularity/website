...
test('additive migration preserves an existing database and restart does not replay seeds', async t => {
  ...
  assert.deepEqual(upgraded.sqlite.prepare('SELECT name FROM local_migrations ORDER BY name').all().map(row => row.name), ['0001_commons.sql', '0002_participations.sql', '0003_work_items.sql', '0004_projects.sql', '0005_receipts.sql', '0006_milestone_reviews.sql', '0007_receipt_completion.sql']);
