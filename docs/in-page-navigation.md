...
A separate outer focus outline remains visible even on the current section link.
...
and reduced motion. In forced colors, check that keyboard focus remains distinct
from the current-section marker. The enhancement reads only section geometry;
it makes no network requests and stores no visitor state.
