...
NAV = (("mission", "Our Mission"), ("observatory", "Observatory"), ("workshop", "Workshop"), ("atlas", "Agent Atlas"), ("lab", "Mission Lab"), ("guide", "Field Guide"), ("connect", "Connect"))
...
def page(slug: str, title: str, description: str, content: str, script: str = "") -> str:
    ...
    extra_style = '<link rel="stylesheet" href="/assets/styles/workshop-v1.css">' if slug == "workshop" else ""
...
<head>
  ...
  <meta property="og:image:alt" content="OSS Singularity event-horizon mark">
...
<section class="observatory-hero" aria-labelledby="observatory-title">
  <div class="observatory-copy"><p class="section-kicker"><span class="status-dot" aria-hidden="true"></span> The commons / Open to every curious mind</p>
    ...
    <p class="hero-note">Find your tools. Connect the dots. Make something that belongs to everyone.</p>
    <div class="hero-actions"><a class="button button-primary" href="/workshop/">Enter the shared Workshop <span aria-hidden="true">↗</span></a><a class="text-link" href="/atlas/">Explore the Atlas →</a></div>
  </div>
...
def atlas(data: dict) -> str:
    labels = {"coding": "Coding agents", "frameworks": "Frameworks", "local": "Local runtimes", "protocols": "Protocols"}
    cards = []
    for i, entry in enumerate(data["entries"], 1):
        ...
        cards.append(f'''<article class="atlas-entry" id="{esc(entry['id'])}" data-category="{esc(entry['category'])}" data-name="{esc(entry['name'])}">
          ...
          <div class="entry-actions"><a class="button button-secondary" href="{esc(entry['website'])}">Explore <span class="sr-only">{esc(entry['name'])}</span> ↗</a><a class="source-link" href="{esc(entry['source_url'])}">Source &amp; license ↗</a><label class="compare-label" hidden><input type="checkbox" value="{esc(entry['id'])}"> Compare <span class="sr-only">{esc(entry['name'])}</span></label></div>
        </article>''')
...
<section class="atlas-controls" aria-label="Find an ecosystem project" hidden>
  ...
  <div class="filter-row" role="group" aria-label="Filter by category"><button type="button" data-filter="all" aria-pressed="true">All signals</button><button type="button" data-filter="coding" aria-pressed="false">Coding agents</button><button type="button" data-filter="frameworks" aria-pressed="false">Frameworks</button><button type="button" data-filter="local" aria-pressed="false">Local runtimes</button><button type="button" data-filter="protocols" aria-pressed="false">Protocols</button></div>
...
<aside class="editorial-note"><h2>A map, with context.</h2><p>These entries serve different purposes. A coding agent is an application; a framework helps you build agents; a runtime serves models; a protocol defines how systems communicate. A listing is not an affiliation, security audit or promise that a tool is right for your data. Licenses and project status can change — check the linked source before adopting.</p><a class="text-link" href="/guide/#building-blocks">Understand the building blocks →</a></aside>
...
def lab() -> str:
    stages = (("observe", "Observe", "Read the mission and its boundary."), ("plan", "Plan", "Choose a small, inspectable next step."), ("build", "Work", "Produce a draft within the allowed scope."), ("review", "Review", "Check evidence against acceptance criteria."), ("handoff", "Handoff", "Bring the result back to a human."))
...
<div class="guide-layout"><aside class="guide-toc"><p class="micro-label">In this field guide</p><a href="#agent-loop">01 · The agent loop</a><a href="#building-blocks">02 · The building blocks</a><a href="#good-mission">03 · A useful mission</a><a href="#trust-boundary">04 · Trust and evidence</a><a href="#first-expedition">05 · Your first expedition</a><a href="#glossary">06 · Decode the vocabulary</a></aside><div class="guide-content">
...
<section class="commons-note"><span class="commons-symbol" aria-hidden="true">✦</span><div><p class="section-kicker">A small beginning. An open horizon.</p><h2>You do not have to see<br>the whole future to help build it.</h2><p>Bring curiosity. Bring care for the people affected by your work. Bring one useful contribution. We can discover the next step together.</p><a class="text-link" href="/workshop/">Find your place in the mission →</a></div></section>
'''
